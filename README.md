...
- The `zksync-os` repository cloned locally. Switch to tag **v0.2.9-interface-v0.0.15**.
- The `zksync-airbender` repository cloned locally
- Download the [evm_replay.bin](https://github.com/matter-labs/zksync-os/releases/download/v0.2.8-simulation-only-interface-v0.0.15/evm_replay.bin) binary and copy into zksync-os/zksync_os directory.
---
...
## Step 1: Witness Generation (`eth_runner`)
...
> **Path Note:** The commands below assume your `zksync-os` and `zksync-airbender` folders share the same parent directory. Adjust the `--bin` path (`../../../zksync-os/zksync_os/evm_replay.bin`) if your setup differs.
